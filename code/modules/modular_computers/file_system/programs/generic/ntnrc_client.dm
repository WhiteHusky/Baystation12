/datum/computer_file/program/chatclient
	...
	filedesc = "DNET Relay Chat Client"
	program_icon_state = "command"
	program_key_state = "med_key"
	program_menu_icon = "comment"
	extended_desc = "This program allows communication over the DNET network"
	size = 8
	requires_ntnet = 1
	requires_ntnet_feature = NTNET_COMMUNICATION
	network_destination = "DNETRC server"
...
/datum/computer_file/program/chatclient/Topic(href, href_list)
	...
	if(href_list["PRG_toggleadmin"])
		...
		if(can_run(usr, 1, access_network))
			if(channel)
				var/response = alert(user, "Really engage admin-mode? You will be disconnected from your current channel!", "DNETRC Admin mode", "Yes", "No")
				if(response == "Yes")
					if(channel)
						channel.remove_client(src)
						channel = null
	...
	if(href_list["PRG_savelog"])
		...
		logfile.stored_data = "\[b\]Logfile dump from DNETRC channel [channel.title]\[/b\]\[BR\]"
...
/datum/nano_module/program/computer_chatclient/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state = GLOB.default_state)
	...
	if (!ui)
		ui = new(user, src, ui_key, "ntnet_chat.tmpl", "DNET Relay Chat Client", 575, 700, state = state)
